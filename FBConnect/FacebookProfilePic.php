 ...
 * Facebook extension may not use these functions, but they're avialable to make it easy to integrate into your
...
class FacebookProfilePic {
	...
	private static $FACEBOOK_USER_AGENT = "Facebook Extension for MediaWiki";
	...
	public static function getImgUrlById($fb_id, $size = "", $userAgent = ""){
		...
		if(empty($userAgent)){
			$userAgent = self::$FACEBOOK_USER_AGENT;
		}
		...
		if( $page ) {
			...
			if(0 < preg_match("/<profile[^>]*>.*?(http[s]?:\/\/[^<]*).*?<\/profile>/is", $page, $matches)){
				...
				wfDebug("Facebook: Could not find a profile picture for user id \"$fb_id\" using url: \"$fqlUrl\"");
			}
		} else {
			wfDebug("Facebook: no content found for facebook fql query: \"$fqlUrl\". Make sure outbound requests are working from your MediaWiki install.\n");
		}

		return $profilePicUrl;
	} // end getImgById()

} // end class FacebookProfilePic
