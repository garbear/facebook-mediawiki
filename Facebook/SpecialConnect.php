...
class SpecialConnect extends SpecialPage {
	...
	public function execute( $par ) {
		...
		wfDebug("Facebook: Executing Special:Connect with the parameter of \"$par\".\n");
		wfDebug('Facebook: User is' . ($wgUser->isLoggedIn() ? '' : ' NOT') . " logged in.\n");
			...
			if ($wgRequest->getCheck('wpCancel')) {
				...
				case 'manual':
					...
					if (!$this->userNameOK($username)) {
						$this->sendPage('chooseNameForm', 'facebook-invalidname');
					} else {
	 ...
	 */
	private function chooseNameForm($messagekey = 'facebook-chooseinstructions') {
